...
const sendLeadsToSheet = require("../service/googleSheetService");
    ...
    async function streamScrape(req, res) {
        ...
        try {
            ...
            // ── GOOGLE SHEET FINAL PUSH ─────────────────────────────
            send(res, "log", {
                message: "Sending verified leads to Google Sheets...",
                type: "info"
            });

            const verifiedLeads = await AmazonLead
                .find({ _id: { $in: verifiedLeadIds } })
                .lean();

            // map ONLY required fields
            const sheetLeads = verifiedLeads.map(lead => ({
                businessName: lead.businessName || "",
                ownerName: lead.ownerName || "",
                phoneNumber: lead.phoneNumber || "",
                address: lead.address || "",
                postcode: lead.postcode || "",
                addressMatch: lead.addressMatch || "",
                ratingPercentage: lead.ratingPercentage || "",
                productUrl: lead.productUrl || "",
                sellerLink: lead.sellerLink || ""
            }));

            // send in batches (safe approach)
            const chunkSize = 30;

            for (let i = 0; i < sheetLeads.length; i += chunkSize) {
                const chunk = sheetLeads.slice(i, i + chunkSize);
                await sendLeadsToSheet(chunk);
            }

            send(res, "log", {
                message: `Google Sheets updated with ${sheetLeads.length} leads`,
                type: "success"
            });
